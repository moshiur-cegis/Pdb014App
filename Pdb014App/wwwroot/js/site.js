...
function MyMessage() {
    this.init = function (msgOpt, title, msg, autoOpen) {
        if (typeof (msgOpt) === "undefined")
            return;
        if (msgOpt.constructor !== String || typeof msgOpt !== "string") {
            title = msgOpt["title"];
            msg = msgOpt["msg"];
            autoOpen = msgOpt["autoOpen"];
            msgOpt = msgOpt["type"];
        }
        var typeIsOk = (msgOpt && msgOpt.replace(/\s/g, "").length > 0);

        $("#msg-title-txt").removeClass("msg-title-txt").addClass("msg-title-txt msg-information");
        $("#msg-title-txt").html($.parseHTML(decodeURI(title)));

        if (typeIsOk)
            $("#msg-body-bg").removeClass().addClass("msg-body-bg msg-" + msgOpt);
...
function MyModal() {
    ...
    this.init = function (title, contentId, autoOpen) {
        $("#modal-title-txt").html($.parseHTML(decodeURI(title)));
    ...
    this.init = function (title, content, autoOpen, isSetBtn) {
        $("#modal-title-txt").html($.parseHTML(decodeURI("▣ " + title)));
        ...
        if (typeof (isSetBtn) !== "undefined" && isSetBtn == true) {
            $("#btnSet").css("display", "");
        } else {
